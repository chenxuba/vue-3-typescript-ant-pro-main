...
const fetchProjectTaskList = async () => {
  ...
  try {
    ...
    // JupyterNotebook项目应该只有一个任务（type=4）
    if (list && list.length > 0) {
      const task = list.find((t: any) => t.type === 4) || list[0]
      ...
      if (task) {
        ...
        if (task.testValidateFiles) {
          evaluationData.value.testValidateFiles = task.testValidateFiles
          // 将评测文件字符串转换为文件列表
          const urls = task.testValidateFiles.split(',')
          testValidateFileList.value = urls.map((url: string, index: number) => ({
            uid: `${index}`,
        ...
        // 回填参考答案数据
        referenceAnswerData.value.showAnswer = task.showAnswer || 1
        referenceAnswerData.value.prohibitCopyAnswer = task.prohibitCopyAnswer || 1
        referenceAnswerData.value.referenceAnswer = task.referenceAnswer || ''
        
        // 标记已保存
        if (task.openTestValidate) {
    ...
    message.warning(error.message || '获取任务列表失败')
  }
}
  ...
  if (currentStep.value === 0) {
    ...
    if (taskId.value) {
      try {
            ...
            Modal.confirm({
              ...
              onOk: async () => {
                try {
                  ...
                  message.error(error.message || '重置文件失败')
                  resolve()
                }
              },
              onCancel: async () => {
        ...
        message.error(error.message || '查询任务用户数失败')
        // 即使查询失败，也允许继续流程
        await handleStopPod()
        currentStep.value = 2
        scrollToTop()
...
<template>
  <div class="edit-jupyter-notebook-page">
    <div class="page-header">
      <h2>编辑JupyterNotebook环境实训项目</h2>
    </div>
    ...
    <a-spin :spinning="loading">
      <div class="page-container">
        ...
        <div class="page-footer">
          ...
          <a-button v-if="currentStep === 2" type="primary" @click="handleNext">完成更新</a-button>
